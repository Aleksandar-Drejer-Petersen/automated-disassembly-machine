"""
press_fail_validation.py

Runs 20 press cycles onto a position that CANNOT be pressed (solid surface),
to validate that the failure detection works correctly.

The press target is placed 20 mm in the -X direction from the red square centre
instead of +X, so the axle presses onto a surface that should not move.
Both sensors and camera vision should consistently report failure.

Workflow:
  1. Home machine
  2. Go to CAM1, find and iteratively centre on the red square
  3. Compute fail target = square_x - 20 mm (same Y), corrected for camera offset
  4. Move axle to fail target
  5. For each of 20 cycles:
       ...
       d. Home Z
       e. Move camera above fail target, capture verify image
       f. Append key metrics row to CSV (including vision result)
       g. Move back to fail target for next cycle
       h. If not last cycle: prompt to replace item

Usage:
    python press_fail_validation.py
    python press_fail_validation.py --cycles 5    # shorter run
"""
...
# ── Settings ──────────────────────────────────────────────────────────────────
PRESS_X_OFFSET_FROM_SQUARE_MM = -20.0  # fail target is 20 mm in -X from square
...
def _centre_on_square(ser, approx_x, approx_y, mm_per_pixel_x, mm_per_pixel_y):
    ...
    for i in range(MAX_CENTER_ITERS):
        tmp = os.path.join(VISION_CURRENT_RUN_DIR, f"pressfailval_centre_iter{i}.jpg")
        capture_image(tmp)
...
def main():
    ...
    out_dir = os.path.join(
        os.path.dirname(__file__), "press_fail_validation_runs", run_ts
    )
    os.makedirs(out_dir, exist_ok=True)
    os.makedirs(VISION_CURRENT_RUN_DIR, exist_ok=True)
    csv_path = os.path.join(out_dir, "press_fail_validation.csv")
    ...
    try:
        ...
        # ── Compute fail target ───────────────────────────────────────────────
        # Fail target = square - 20 mm in X (solid surface, should not move)
        # Apply camera-to-bit offset so the axle sits over the target
        press_x = rs_x + PRESS_X_OFFSET_FROM_SQUARE_MM - CAMERA_OFFSET_X_MM
        press_y = rs_y - CAMERA_OFFSET_Y_MM
        print(f"Fail target (bit coords): X={press_x:.3f}  Y={press_y:.3f}")

        # ── Move to fail target ───────────────────────────────────────────────
        print("\n=== Moving to fail target ===")
        ...
        for cycle in range(1, n_cycles + 1):
            ...
            # Camera verification: move camera above fail target and check
            ...
            # For fail validation: correct = sensors say FAILED and vision says FAILED
            sensor_str = "SUCCESS" if ok else "FAILED"
            vision_str = ("VISION OK" if vision_ok else
                          "VISION FAILED" if vision_ok is not None else
                          "VISION ERROR")
            correctly_detected = (not ok) and (vision_ok is False)
            ...
            # Move back to fail target so the axle is positioned for the next cycle
            print(f"  Returning to fail target (X={press_x:.3f} Y={press_y:.3f})...")
            send_command(ser, f"to {press_x:.3f} {press_y:.3f} 0.000")
            wait_for_message(ser, "SYSTEM IS AT POSITION", timeout=60)
        ...
        # ── Summary ───────────────────────────────────────────────────────────
        print(f"\n{'='*55}")
        print(f"  FAIL VALIDATION COMPLETE: {correct_failures}/{n_cycles} cycles correctly detected as failure")
        print(f"  CSV  → {csv_path}")
