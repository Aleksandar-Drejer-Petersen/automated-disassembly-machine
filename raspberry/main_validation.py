...
Same pipeline as main.py, with a post-run accuracy validation layer.

After all operations execute and the camera vision pass completes:
  1. Prints a full per-operation result table.
  2. Asks the operator two questions:
       a. How many operations actually failed in total?
       b. How many operations were misclassified (wrong bin — said fail but
          was OK, or said OK but was fail)?
  3. Appends one timestamped row to main_validation_results.csv (creates it
     on first run, appends on every subsequent run).

CSV columns:
  timestamp, total_ops, press_ops, screw_ops,
  system_screw_fails, system_vision_ok, system_vision_fail, system_vision_error,
  system_total_reported_fails,
  operator_confirmed_system_fails,   <- how many of the system fails were real
  operator_actual_fails,             <- ground truth total failures
  true_fails,                        <- operator_actual_fails - EXPECTED_ALWAYS_FAIL
  fail_precision,                    <- confirmed / system_reported  (avoids false positives)
  fail_recall                        <- confirmed / operator_actual  (avoids false negatives)

Usage:
    python main_validation.py
    python main_validation.py --no-execute
"""
...
# Two buttons are always expected to fail (unpressable by design).
# true_fails = operator_actual_fails - EXPECTED_ALWAYS_FAIL
    ...
    while improved:
        ...
        for i in range(n - 1):
            for j in range(i + 2, n):
                # Use start_xy as the left anchor when reversing from index 0
                left  = start_xy if (i == 0 and start_xy is not None) else coords[path[i]]
                # Use end_xy as the right anchor when reversing to the last index
                right = (end_xy if end_xy is not None else
                         (coords[path[j + 1]] if j + 1 < n else None))
...
def plot_tsp_path(segments, save_path, total_dist=None):
    """
    segments : list of (title, waypoints) — one panel per entry.
               waypoints = [(x, y, label, wtype), ...]
    Saves a stitched figure with each panel cropped to its own content.
    Returns total Euclidean travel distance in mm.
    """
    ...
    PAD = 18   # mm padding around each panel's content
    n   = len(segments)
...
def _laser_recovery(ser):
    ...
    try:
        ...
        stop.set()
        t.join(timeout=1.0)
        print()   # newline after the \r laser line
        _write("LASER STOP")
        time.sleep(0.15)
        ser.reset_input_buffer()
...
def main():
    ...
    try:
        ...
        # ── Camera sweep: detect all operations ───────────────────────────────
        all_operations  = []
        visited_squares = []
        sweep_waypoints = [(0.0, 0.0, "HOME", "home")]
        last_sweep_pos  = (0.0, 0.0)
        op_index  = 1
        sq_global = 0
        ...
        for cam_name, cam_x, cam_y in CAMERA_SWEEP:
            ...
            # TSP-order detection visits within this camera view
            if len(new_squares) > 1:
                new_squares = [new_squares[i]
                               for i in tsp(new_squares, start_xy=(cam_x, cam_y))]
        ...
        # ── Plan execution order: TSP on bit groups + ops within each group ──
        bit_order = []
        seen = set()
        for op in all_operations:
        ...
        # TSP-order which bit group to run first, starting from the last sweep position
        if len(bit_order) > 1:
            rack_coords = [BIT_RACK_XY.get(t, (0.0, 0.0)) for t in bit_order]
            bit_order  = [bit_order[i] for i in tsp(rack_coords, start_xy=last_sweep_pos)]

        # TSP-order ops within each bit group (start from that bit's rack)
        ordered_groups = {}
        for bit_name in bit_order:
            ops       = grouped[bit_name]
            op_coords = [(op["abs_x"], op["abs_y"]) for op in ops]
            rack_xy   = BIT_RACK_XY.get(bit_name, (0.0, 0.0))
            ordered_groups[bit_name] = [
                ops[i] for i in tsp(op_coords, start_xy=rack_xy, end_xy=rack_xy)
            ]
        ...
        last_bit_rack = BIT_RACK_XY.get(bit_order[-1], (0.0, 0.0)) if bit_order else (0.0, 0.0)
        if seen_sq:
            sq_cam_coords = [(press_by_square[sq][0]["cam_x"],
                              press_by_square[sq][0]["cam_y"]) for sq in seen_sq]
            sq_order = [seen_sq[i] for i in tsp(sq_cam_coords, start_xy=last_bit_rack)]
        else:
            sq_order = []

        # ── Build plot segments and generate TSP path plot ───────────────────
        # Segment 1: camera sweep only (home → cams → detection squares)
        sweep_panel_wps = list(sweep_waypoints)

        # Middle segments: one panel per bit
        bit_segments = []
        for bit_name in bit_order:
        ...
        # Last segment: vision verification (starts from last bit rack → verify cams → home)
        last_bit_name = bit_order[-1] if bit_order else None
        lrx, lry = BIT_RACK_XY.get(last_bit_name, (0.0, 0.0)) if last_bit_name else (0.0, 0.0)
        verify_panel_wps = [
            (lrx, lry, f"PLACE\n{last_bit_name}" if last_bit_name else "RACK", "rack")
        ]
        for sq_id in sq_order:
            sq_ops = press_by_square[sq_id]
            verify_panel_wps.append((sq_ops[0]["cam_x"], sq_ops[0]["cam_y"],
                                     f"VER\nsq{sq_id}", "verify"))
        verify_panel_wps.append((0.0, 0.0, "HOME", "home"))

        plot_segments = (
            [("Sweep", sweep_panel_wps)]
            + bit_segments
            + [("Vision Verify", verify_panel_wps)]
        )

        # True total distance: every mm from home → sweep → bits → verify → home,
        # including all inter-segment transitions.
        _seq = list(sweep_waypoints)
        for _, bit_wps in bit_segments:
            _seq.extend(bit_wps)
        for sq_id in sq_order:
            sq_ops = press_by_square[sq_id]
            _seq.append((sq_ops[0]["cam_x"], sq_ops[0]["cam_y"],
                         f"VER\nsq{sq_id}", "verify"))
        _seq.append((0.0, 0.0, "HOME", "home"))
        ...
        for bit_name in bit_order:
            ...
            grab_result = "UNKNOWN COMMAND"
            for _attempt in range(3):
                if _attempt:
                    print(f"  Retrying grab (attempt {_attempt + 1}/3)...")
                send_command(ser, f"grab {bit_name.lower()}")
                grab_result = wait_for_any_message(
                    ser, ["GRAB: complete.", "GRAB ERROR"], timeout=90
                )
                if grab_result != "UNKNOWN COMMAND":
                    break
            if grab_result is None or "ERROR" in grab_result or grab_result == "UNKNOWN COMMAND":
            ...
            print(f"\n=== Placing {bit_name} ===")
            place_result = "UNKNOWN COMMAND"
            for _attempt in range(3):
                if _attempt:
                    print(f"  Retrying place (attempt {_attempt + 1}/3)...")
                send_command(ser, f"place {bit_name.lower()}")
                place_result = wait_for_any_message(
                    ser, ["PLACE: complete.", "PLACE ERROR"], timeout=90
                )
                if place_result != "UNKNOWN COMMAND":
                    break
            if place_result is None or "ERROR" in place_result or place_result == "UNKNOWN COMMAND":
